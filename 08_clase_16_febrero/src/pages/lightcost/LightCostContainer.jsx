...
function LightCostContainer() {
  ...
  const lightCostArray = Object.entries(useLoaderData()).map((item) => item[1]);
  ...
  const renderLightCost = lightCostArray.map((item, index) => {
    if (mostCheapest.includes(item)) {
      return <LightCostCard avgCost={avgCost} key={index} item={item} cheap />;
    } else if (mostExpensive.includes(item)) {
      return (
        <LightCostCard avgCost={avgCost} key={index} item={item} expensive />
      );
    } else {
      return (
        <LightCostCard avgCost={avgCost} key={index} item={item} average />
      );
    }
  });
  ...
  return (
    <>
      <header className="my-3 text-2xl text-center font-header">
        ...
        <h2 className="text-lg font-bold text-center">Avg: {avgCost} €/Mwh</h2>
      </header>
